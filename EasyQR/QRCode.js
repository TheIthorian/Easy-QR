...
const ERROR_LOOKUP = {
    ...
    ERC_ALPHANUMERIC_MODE_INVALID_DATA:
        'Alphanumeric mode may only contain digits and upper-case letters',
    ...
    ERC_VERSION_NOT_SUPPORTED:
        'QR version with more than one error corretion group is not supported',
};
...
export class QRCode {
    ...
    */
    ...
    getGroup1(output, index) {
        ...
        for (
            let i = 0;
            i < VERSION_DATA[this.correctionLevel].blocksInGroup1[this.version - 1];
            i++
        ) {
            group1.push([]);
            ...
            for (
                let j = 0;
                j < VERSION_DATA[this.correctionLevel].codewordsInGroup1Block[this.version - 1];
                j++
            ) {
    ...
    getGroup2(output, index) {
        ...
        for (
            let i = 0;
            i < VERSION_DATA[this.correctionLevel].blocksInGroup2[this.version - 1];
            i++
        ) {
            group2.push([]);
            ...
            for (
                let j = 0;
                j < VERSION_DATA[this.correctionLevel].codewordsInGroup2Block[this.version - 1];
                j++
            ) {
                // Add the codeword to the block
    ...
    getNumberOfErrorCorrectionCodeWords() {
        return (
            VERSION_DATA[this.correctionLevel].ECCodeWordsPerBlock[this.version - 1] *
    ...
    getTotalNumberOfCodewords() {
        ...
        let codewordsInGroup1Block =
            VERSION_DATA[this.correctionLevel].codewordsInGroup1Block[this.version - 1];
        let blocksInGroup2 = VERSION_DATA[this.correctionLevel].blocksInGroup2[this.version - 1];
        let codewordsInGroup2Block =
            VERSION_DATA[this.correctionLevel].codewordsInGroup2Block[this.version - 1];
